...
@test "arbiter-health: extracts mons when monmap is NOT last JSON field" {
  ...
  local mons_json
  mons_json=$(printf '%s' "$json" | awk -v Q='"' '
    {
      needle = Q "mons" Q ":["
      pos = index($0, needle)
      if (pos == 0) exit
  ...
  # Should find 3 mons
  local count
  count=$(printf '%s' "$mons_json" | grep -o '"name"' | wc -l | tr -d ' ')
  [ "$count" -eq 3 ]

  # Should find all three names
  local names
  names=$(printf '%s' "$mons_json" | grep -o '"name":"[^"]*"' | sed 's/"name":"//g; s/"//g' | tr '\n' ',' | sed 's/,$//')
  [ "$names" = "a,b,ext-c" ]
}
...
@test "arbiter-health: extracts mons when monmap IS last JSON field" {
  ...
  local names
  names=$(printf '%s' "$mons_json" | grep -o '"name":"[^"]*"' | sed 's/"name":"//g; s/"//g')
  [ "$names" = "solo" ]
}
