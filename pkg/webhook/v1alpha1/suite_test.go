...
var _ = BeforeSuite(func() {
	...
	testEnv = &envtest.Environment{
		CRDDirectoryPaths:     []string{filepath.Join("..", "..", "..", "contrib", "k8s", "crd")},
		ErrorIfCRDPathMissing: true,

		WebhookInstallOptions: envtest.WebhookInstallOptions{
			Paths: []string{filepath.Join("..", "..", "..", "contrib", "k8s", "webhook")},
		},
	}
	...
	err = SetupRemoteClusterWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = SetupRemoteArbiterWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())
